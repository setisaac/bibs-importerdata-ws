...
public class TaskImporterWS implements Task, ManagedLifecycle {
	...
	private static AppReports appReports;

	private String incialMonth;
	private Integer cantMonth;
	...
	public void execute() {
		try {
			...
			appReports = new AppReports(incialMonth, cantMonth);
			JSONArray jsonArrayData = appReports.getJsonArray();
	...
	*/
	...
	private static JSONObject sendPost(final String url) throws Exception {
		/*
		 * System.out.println( "Hello World!" ); FirefoxProfile profile = new
		 * FirefoxProfile(); WebDriver driver = new FirefoxDriver(new
		 * FirefoxBinary(new File( pathFirefox)), profile); System.out.println(
		 * "Instanciado!" );
		 */
